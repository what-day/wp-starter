( function ( $ ) {
	CartFlowsAdminEdit = {
		...
		_bind: function () {
			$( document ).on(
				'change',
				'.wcf-field-font-family',
				CartFlowsAdminEdit._set_font_weight_select_options
			);
			$( document ).on(
				'change',
				'.wcf-field-font-weight',
				CartFlowsAdminEdit._set_font_weight_val
			);
		},
				...
				if ( wcf.google_fonts[ font_family ] ) {
					...
					$.each( variants, function ( index, weight ) {
						if ( ! weight.includes( 'italic' ) ) {
							new_font_weights[ weight ] =
								wcf.font_weights[ weight ];
						}
					} );

					var weight = $(
						'.wcf-field-font-weight[data-for="' + id + '"]'
					);
					if ( weight.length ) {
					...
					var variants =
						wcf.system_fonts[ font_family ][ 'variants' ];
		...
		_set_google_url: function () {
			...
			$( '#wcf-field-google-font-url' ).val(
				'//fonts.googleapis.com/css?family=' + google_url
			);
		},
...
( function ( $ ) {
	...
	var wcf_woo_product_search_init = function () {
		var $product_search = $(
			'.wcf-product-search:not(.wc-product-search)'
		);
						...
						ajax: {
							...
							data: function ( params, page ) {
								return {
									...
									security:
										cartflows_admin.wcf_json_search_products_and_variations_nonce,
								};
							},
					...
					ajax: {
						...
						data: function ( params, page ) {
							return {
								...
								security:
									cartflows_admin.wcf_json_search_coupons_nonce,
							};
						},
						processResults: function ( data, page ) {
							var terms = [];
				...
				ajax: {
					...
					data: function ( params, page ) {
						return {
							...
							security:
								cartflows_admin.wcf_json_search_pages_nonce,
						};
					},
					processResults: function ( data, page ) {
						return { results: data };
	...
	var wcf_hide_discount_value_field = function () {
		var show_hide_discount_field = function ( $this, value ) {
			if ( '' === value ) {
				$this
					.closest( '.wcf-repeatable-row-advance-fields' )
					.find( '.wcf-repeatable-row-discount-field' )
					.hide();
			} else {
				$this
					.closest( '.wcf-repeatable-row-advance-fields' )
		...
		wrap.find( '.wcf-repeatable-discount-type-field select' ).each(
			function ( e ) {
				$this = $( this );
				var value = $this.val();
		...
		$( document ).on(
			'change',
			'.wcf-repeatable-discount-type-field select',
			function ( e ) {
				$this = $( this );
				var value = $this.val();
				show_hide_discount_field( $this, value );
		...
		$( '.wcf-repeatable-row-advance-fields input[type=number]' ).on(
			'keyup',
			function ( e ) {
				var value = $( this ).val();
		...
		$( document.body ).on(
			...
			function ( e ) {
				...
				if (
					...
					! e_target.hasClass( 'wcf-repeater-fields-head-wrap' )
				) {
					return;
				}
	...
	var wcf_add_repeatable_product = function () {
		$( '.wcf-add-repeatable' ).on( 'click', function ( e ) {
			var $this = $( this ),
				...
				wrap = $this
					.closest( '.wcf-field-row-content' )
					.find( '.wcf-repeatables-wrap' ),
	...
	var wcf_set_variation_mode_option = function () {
		...
		$( document ).on( 'change', '.wcf-variation-mode select', function (
			e
		) {
			var $this = $( this ),
				variation_mode = $this.val(),
	...
	var wcf_custom_fields_events = function () {
		...
		/* Change Custom Field*/
		$(
			'.wcf-column-right .wcf-checkout-custom-fields .wcf-cc-fields .wcf-cc-checkbox-field input:checkbox'
	...
	var wcf_custom_fields = function () {
		var wrap = $( '.wcf-checkout-table' ),
			custom_fields = wrap.find(
				'.wcf-column-right .wcf-checkout-custom-fields .wcf-cc-fields .wcf-cc-checkbox-field .field-wcf-custom-checkout-fields input:checkbox'
			);
	...
	var wcf_settings_tab = function () {
		if ( $( '.wcf-metabox-wrap' ).length ) {
			$( '.wcf-metabox-wrap' ).each( function ( index ) {
				let $this = $( this ),
					...
					save_settings = wcf_local_storage.get(
						'wcf_meta_settings'
					),
				...
				if ( save_settings[ settings_id ] !== undefined ) {
					...
					let tab = save_settings[ settings_id ],
						tab_class = parent.find(
							'.wcf-tab[data-tab="' + tab + '"]'
						),
		...
		$( '.wcf-tab' ).on( 'click', function ( e ) {
			...
			wcf_update_settings_local_storage(
				settings_id,
				$this.data( 'tab' )
			);
	...
	var wcf_input_file_init = function () {
		...
		$( document.body ).on( 'click', '.wcf-select-image', function ( e ) {
			...
			file_frame.on( 'select', function () {
				...
				window.inputWrapper
					.find( '#wcf-image-preview' )
					.children( '.saved-image' )
					.remove();
				...
				window.inputWrapper
					...
					.append(
						'<img src="' +
							attachment.url +
							'" width="150" class="saved-image" style="margin-bottom:12px;" />'
					);
	...
	var wcf_checkout_thankyou_hide_show_init = function () {
		...
		var wcf_checkout_advance_style_fields = function () {
			var wrap = $( '.wcf-checkout-design-table' ),
				custom_fields = wrap.find(
					'.wcf-column-right .wcf-checkout-style .wcf-cs-fields .field-wcf-advance-options-fields input:checkbox'
				);
		...
		var wcf_thankyou_advance_style_fields = function () {
			var wrap = $( '.wcf-thankyou-design-table' ),
				checkbox_field = $(
					'.wcf-thankyou-design-table [name="wcf-tq-advance-options-fields"]'
				);
		...
		var wcf_thankyou_settings_fields = function () {
			var wrap = $( '.wcf-thankyou-table' ),
				checkbox_field = $(
					'.wcf-thankyou-table [name="wcf-show-tq-redirect-section"]'
				);

			var field_names = [ '.field-wcf-tq-redirect-link' ];
		...
		var wcf_advance_style_fields_events = function () {
			...
			$( '.wcf-thankyou-table [name="wcf-show-tq-redirect-section"]' ).on(
				'change',
				function ( e ) {
					wcf_thankyou_settings_fields();
				}
