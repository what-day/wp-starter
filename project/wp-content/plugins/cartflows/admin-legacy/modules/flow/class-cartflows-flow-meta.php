...
class Cartflows_Flow_Meta {
	...
	public function admin_notices() {
		...
		do_action( 'wcf_back_to_flow_button_after', $flow_id );
	}
	...
	public function admin_scripts() {
		...
		if ( ( 'post-new.php' == $pagenow || 'post.php' == $pagenow ) && CARTFLOWS_FLOW_POST_TYPE == $screen->post_type ) {
			...
			wp_enqueue_script(
				'wcf-flow-meta',
				CARTFLOWS_URL . 'admin-legacy/assets/js/flow-admin-edit.js',
				array( 'jquery', 'jquery-ui-sortable' ),
				CARTFLOWS_VER,
				true
			);

			wp_enqueue_style( 'wcf-flow-meta', CARTFLOWS_URL . 'admin-legacy/assets/css/flow-admin-edit.css', '', CARTFLOWS_VER );
