...
if ( ! class_exists( 'CartFlows_Legacy_Importer' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class CartFlows_Legacy_Importer {
		...
		public function exporter_markup() {
			include_once CARTFLOWS_LEGACY_ADMIN_DIR . 'includes/exporter.php';
		}
		...
		public function importer_markup() {
			include_once CARTFLOWS_LEGACY_ADMIN_DIR . 'includes/importer.php';
		}
		 ...
		 *
		 * @param array $flow Flows array.
		 * @param bool  $return Is return.
		 * @return array
		 */
						...
						<div class="inner">
							<div class="template">
								<span class="thumbnail site-preview cartflows-preview-flow-steps" data-flow-steps="{{ JSON.stringify( data.items[ key ].flow_steps ) }}" data-title="{{ data.items[ key ].title.rendered }}">
									...
									<# } else { #>
											<a href="<?php echo CartFlows_API::get_instance()->get_site_url() . 'preview/?'; ?>flow={{ data.items[ key ].id }}&title={{{ data.items[ key ].title.rendered }}}" class="preview" target="_blank">Preview <i class="dashicons dashicons-external"></i></a>
									<# } #>
						...
						<div class="inner">
							<div class="template">
								<span class="thumbnail site-preview cartflows-preview-flow-steps" data-flow-steps="{{ JSON.stringify( data.items[ key ].flow_steps ) }}" data-title="{{ data.items[ key ].title.rendered }}">
									...
									<# } else { #>
											<a href="<?php echo CartFlows_API::get_instance()->get_site_url() . 'preview/?'; ?>step={{ data.items[ key ].id }}&title={{{ data.items[ key ].title.rendered }}}" class="preview" target="_blank">Preview <i class="dashicons dashicons-external"></i></a>
									<# } #>
		...
		public function scripts( $hook = '' ) {
			...
			$localize_vars = array(
				...
				'server_url'               => CartFlows_API::get_instance()->get_site_url(),
				'server_rest_url'          => CartFlows_API::get_instance()->get_site_url() . 'wp-json/wp/v2/',
				'site_url'                 => site_url(),
	 ...
	 */
	CartFlows_Legacy_Importer::get_instance();
